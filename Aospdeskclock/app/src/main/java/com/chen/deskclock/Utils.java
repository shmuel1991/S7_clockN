...
public class Utils {
    ...
    /**
     * @return {@code true} if the device is {@link Build.VERSION_CODES#S} or later
     */
    ...
    /**
     * Combine the given PendingIntent {@code flags} with the mutability flag required by the
     * platform. Android 12 (S) and later require every PendingIntent to explicitly declare
     * either {@link PendingIntent#FLAG_IMMUTABLE} or {@link PendingIntent#FLAG_MUTABLE}; all of
     * this app's PendingIntents are immutable.
     *
     * @param flags the base PendingIntent flags (e.g. {@link PendingIntent#FLAG_UPDATE_CURRENT})
     * @return the flags with {@link PendingIntent#FLAG_IMMUTABLE} added on Android S+
     */
    ...
    /**
     * Registers a {@link BroadcastReceiver} for context-registered (non-manifest) receivers.
     * Android 13 (Tiramisu) and later require every context-registered receiver to explicitly
     * declare whether it is exported. All of this app's dynamic receivers listen to system or
     * in-app broadcasts only, so they are registered as not-exported. Failing to pass the flag
     * crashes the app on Android 13+ (e.g. when starting the screensaver).
     */
    ...
    public static PendingIntent pendingServiceIntent(Context context, Intent intent) {
        return PendingIntent.getService(context, 0, intent, pendingIntentFlags(FLAG_UPDATE_CURRENT));
    }
    ...
    public static PendingIntent pendingActivityIntent(Context context, Intent intent) {
        return PendingIntent.getActivity(context, 0, intent, pendingIntentFlags(FLAG_UPDATE_CURRENT));
    }
