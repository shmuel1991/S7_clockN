...
final class SilentSettingsModel {
    ...
    SilentSettingsModel(Context context, NotificationModel notificationModel) {
        ...
        if (Utils.isMOrLater()) {
            ...
            Utils.registerReceiver(context, new DoNotDisturbChangeReceiver(), filter);
        }
    }
    ...
    private final class CheckSilenceSettingsTask extends AsyncTask<Void, Void, SilentSetting> {
        ...
        protected SilentSetting doInBackground(Void... parameters) {
            ...
            // Note: blocked-notification detection intentionally omitted. The POST_NOTIFICATIONS
            // permission is requested via the standard runtime permission dialog instead of
            // surfacing a snackbar that opens a settings screen.
            return null;
        }
