...
import com.chen.deskclock.FabContainer;
import com.chen.deskclock.FormattedTextUtils;
...
public class TimerSetupView extends LinearLayout implements View.OnClickListener,
    ...
    /** Invoked when the user confirms the entered duration with the OK/Enter hardware key. */
    private Runnable mStartTimerAction;
    ...
    public TimerSetupView(Context context, AttributeSet attrs) {
        ...
        // The on-screen keypad has been removed; the duration is entered using the device's
        // hardware keys, so this view must be able to take focus and receive key events.
        setFocusable(true);
    ...
    /**
     * @param action invoked when the user presses the OK/Enter hardware key with valid input
     */
    public void setStartTimerAction(Runnable action) {
        mStartTimerAction = action;
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_DEL || keyCode == KeyEvent.KEYCODE_FORWARD_DEL) {
            ...
            view = mDigitViews[keyCode - KeyEvent.KEYCODE_NUMPAD_0];
        } else if (keyCode == KeyEvent.KEYCODE_ENTER
                || keyCode == KeyEvent.KEYCODE_NUMPAD_ENTER
                || keyCode == KeyEvent.KEYCODE_DPAD_CENTER) {
            // The OK/Enter key starts the timer when a valid duration has been entered.
            if (hasValidInput() && mStartTimerAction != null) {
                mStartTimerAction.run();
            }
            return true;
        }
